...
/**
 * Serwer WebSocket testowy — MINIMALNA implementacja RFC 6455 (handshake + ramki tekstowe) na
 * surowym `ServerSocket`, WYŁĄCZNIE w kodzie testowym (ADR-0035, §Konsekwencje: „akceptowalne, bo
 * nie jest to Public API, wyłącznie narzędzie weryfikacyjne, analogicznie do GreenMail używanego
 * w testach Adaptera Email"). Nie obsługuje fragmentacji ramek ani rozszerzeń — wystarczające dla
 * scenariuszy end-to-end Iteracji 7.5 (pojedyncze ramki tekstowe, zamknięcie połączenia).
 */
class TestWebSocketServer {
    ...
    /** Wysyła ramkę tekstową do WSZYSTKICH aktualnie podłączonych klientów. */
    fun broadcastText(text: String) {
        connections.forEach { it.sendText(text) }
    }

    /** Zamyka wszystkie aktywne połączenia od strony serwera (symuluje utratę połączenia). */
    ...
    private class Connection(
            ...
            try {
                while (!socket.isClosed) {
                    ...
                    when (frame.opcode) {
                        ...
                        0x8 -> { close(); break }
                        0x9 -> sendFrame(0xA, frame.payload) // pong w odpowiedzi na ping
                    }
                }
            } catch (_: Exception) {
        ...
        /** Ramki serwer→klient NIGDY nie są maskowane (RFC 6455). */
